...
export async function GET(_req: NextRequest) {
  ...
  try {
    ...
    // Fire-and-forget cleanup — don't block dashboard stats on this RPC
    void supabase.rpc('cleanup_orphaned_orders', { expiry_hours: 24 })
      .then(({ error }) => { if (error) console.error('[Dashboard] Order cleanup failed:', error) })
