/**
 * Add a R195 setup_fee pricing rule to every active product group
 * that does not already have one.
 *
 * Usage:
 *   node scripts/add-setup-fee.mjs --dry-run   # preview only
 *   node scripts/add-setup-fee.mjs             # apply to DB
 */
...
async function main() {
  // 1. Fetch all active product groups
  ...
  for (const g of groups) {
    // Check if a setup_fee rule already exists for this group
  ...
  console.log(`\n${DRY_RUN ? '[DRY RUN] Would insert' : 'Inserting'} R${SETUP_FEE} setup_fee for ${toInsert.length} product(s).`)
  ...
  console.log(`\nDone! R${SETUP_FEE} setup fee added to ${toInsert.length} product group(s).`)
}
